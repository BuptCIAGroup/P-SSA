...
ifeq ($(mode),release)
	CXXFLAGS = -I/opt/cuda-10.1/include/ -I/usr/local/hdf5/include/ -I../flann-master/src/cpp/ -pipe -std=c++11 -Wall -pedantic -DNDEBUG -O2 -mtune=native -march=native
	NVCCFLAGS = -std=c++11 -m64 -arch=sm_61 -Xptxas="-v"  -DNDEBUG
else
   mode = debug
	CXXFLAGS = -I/opt/cuda-10.1/include/ -I/usr/local/hdf5/include/ -I../flann-master/src/cpp/ -g -pipe -std=c++11 -Wall -O0 -Wcast-align  -Wctor-dtor-privacy -Wdisabled-optimization -Wformat=2 -Winit-self -Wlogical-op  -Wmissing-include-dirs -Wnoexcept -Woverloaded-virtual -Wredundant-decls   -Wsign-promo -Wstrict-null-sentinel -Wstrict-overflow=5 -Wswitch-default -Wundef -Werror -Wno-unused
	NVCCFLAGS = -G -std=c++11 -m64 -arch=sm_61
endif

# This should point to a valid CUDA lib64/ location
LDFLAGS  = -L/opt/cuda-10.1/lib64/ -lcuda -lcudart -lcusparse -lcudadevrt -L/usr/local/lib/ -llz4 

BUILDDIR = obj
TARGET = gpussa
SRCDIR = src
SOURCES = main.cc ssa.cc common.cc stopcondition.cc 
CUDA_SOURCES = gpu_ssa.cu  tsp_ls_gpu.cu cuda_utils.cu 
